...
// var pointerList sync.Map = sync.Map{}
...
func Stringify[T any](v *T, key ...string) (string, error) {
	ps := "&" + encodeType(*v) + "*" + strconv.FormatUint(uint64(uintptr(unsafe.Pointer(v))), 36)

	pListMu.Lock()
	// pointerList.Store(ps, v)
	pointerList[ps] = v
	pListMu.Unlock()

	if len(key) != 0 && len(key[0]) != 0 {
		enc, err := encrypt([]byte(ps), []byte(key[0]))
		if err != nil {
			return "", err
		}
		ps = "&"+encodeToString(enc)
	}else{
		key := RandBytes(16)
		enc, err := encrypt([]byte(ps), []byte(key))
		if err != nil {
			return "", err
		}
		ps = "&"+encodeToString(enc)+"#"+key
	}
...
func Parse[T any](ps string, key ...string) *T {
	...
	if len(data) == 0 || len(data[0]) == 0 || ((len(key) == 0 || len(key[0]) == 0) && (len(data) != 2 || len(data[1]) == 0)) {
		return nil
	}
